...
//
// Big-O Analysis:
//   Channel recv: O(1) — lock-free MPSC dequeue.
//   Batch INSERT:  O(n log n) amortised — n entries × B-tree update per index.
//                  n is bounded by 1024, so this is effectively O(k) in practice.
//   RAM ceiling:   1024 entries × 256 bytes avg = ~256KB per batch.
//                  4096-entry channel backpressure = ~1MB maximum buffer.
...
// On dev (Windows / non-Linux), write to a local path instead.
...
/// Runs the `CREATE TABLE IF NOT EXISTS activity_log` migration once.
/// Called lazily on first flush — avoids blocking the boot critical path.
///
/// Schema uses INTEGER types for `level`, `module`, and `tags` so SQLite's
/// single-byte comparison path is used for indexed queries — O(log n + k)
...
// ──────────────────────────────────────────────────────────────────────────────
// Dedicated SQLite DB Writer Thread & Async Loop (the Tokio task)
// ──────────────────────────────────────────────────────────────────────────────

/// Background Tokio task — runs until the MPSC sender is dropped (Governor shutdown).
///
/// Spawns a dedicated database writer thread to serialize all SQL operations on a
/// single thread, keeping the SQLite connection open and caching the prepared
/// insert statement.
pub async fn flush_loop(
    ...
    // Spawn the dedicated SQLite database writer thread
    ...
    std::thread::spawn(move || {
        ...
        // Enable WAL mode for concurrent readers without blocking writes.
        ...
        tracing::info!(subsystem = "log_flush", "Database writer thread initialized successfully");
        ...
        while let Ok(batch) = db_rx.recv() {
...

            {
                let mut stmt = match tx.prepare_cached(
                    ...
                    Ok(s) => s,
                    Err(e) => {
                        tracing::error!(subsystem = "log_flush", "Failed to prepare cached statement: {}", e);
                        continue;
                    }
                };

                for entry in &batch {
                    let custom_tags_json = entry.custom_tags.as_ref()
                        .and_then(|v| serde_json::to_string(v).ok());
                    let telemetry_blob = entry.telemetry.as_ref()
            ...
            } // Drop stmt to release mutable borrow on tx/connection before commit
    ...
    loop {
        ...
        match timeout(timeout_dur, log_rx.recv()).await {
            Ok(Some(entry)) => {
                // Fan-out to live SSE stream subscribers (broadcast — ignore lag errors)
                let _ = log_broadcast_tx.send(entry.clone());

                batch.push(entry);

                // Trigger 1: High-water mark reached — flush immediately
                if batch.len() >= BATCH_HIGH_WATER_MARK {
                    let drained = std::mem::replace(&mut batch, Vec::with_capacity(BATCH_HIGH_WATER_MARK));
                    let _ = db_tx.send(drained);
                    deadline = Instant::now() + Duration::from_millis(FLUSH_INTERVAL_MS);
                }
            }
            Ok(None) => {
                // MPSC sender was dropped — Governor is shutting down. Drain remainder.
                if !batch.is_empty() {
                    let _ = db_tx.send(batch);
                }
                tracing::info!(subsystem = "log_flush", "Log flush task terminated (channel closed)");
                break;
            }
            Err(_) => {
                // Trigger 2: 500ms timeout expired
                if !batch.is_empty() {
                    let drained = std::mem::replace(&mut batch, Vec::with_capacity(BATCH_HIGH_WATER_MARK));
                    let _ = db_tx.send(drained);
                }
