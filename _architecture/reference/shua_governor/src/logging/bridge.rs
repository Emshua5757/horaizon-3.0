...
use crate::logging::entry::{LogEntry, LEVEL_INFO, LEVEL_WARN, LEVEL_ERROR, LEVEL_DEBUG, LEVEL_TRACE};
use tokio::sync::mpsc;
    ...
    S: Subscriber,
{
    ...
    fn on_event(&self, event: &tracing::Event<'_>, _ctx: Context<'_, S>) {
        ...
        // Extract message, subsystem, and other fields
        struct Visitor {
            msg: String,
            subsystem: String,
        ...
        impl tracing::field::Visit for Visitor {
            fn record_debug(&mut self, field: &tracing::field::Field, value: &dyn std::fmt::Debug) {
                if field.name() == "message" {
                    ...
                    let _ = self.telemetry.insert(field.name().to_string(), serde_json::json!(format!("{:?}", value)));
                }
            }
            fn record_str(&mut self, field: &tracing::field::Field, value: &str) {
                if field.name() == "message" {
                    ...
                    let _ = self.telemetry.insert(field.name().to_string(), serde_json::json!(value));
                }
            }
        }
        ...
        let entry = LogEntry {
            ...
            tags: 1, // SYSTEM
            custom_tags: None,
            telemetry: if visitor.telemetry.is_empty() { None } else { Some(serde_json::Value::Object(visitor.telemetry)) },
            trace_id: None,
        };
