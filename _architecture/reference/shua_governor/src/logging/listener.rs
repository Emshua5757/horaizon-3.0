...
// Bypasses stdout redirection, console block-buffering, and formatting lag.
...
/// Main entry point for the background IPC log listener.
/// Binds to UDS on Linux (/tmp/horaizon_logs.sock) and TCP on Windows (127.0.0.1:5001).
pub async fn start_log_ipc_listener(log_tx: mpsc::Sender<LogEntry>) {
    // 1. Spawn TCP loopback listener (runs on both Windows and Linux)
    ...
    // 2. Spawn UDS listener (Linux only)
    #[cfg(target_os = "linux")]
    {
        ...
        let listener = match tokio::net::UnixListener::bind(socket_path) {
            Ok(l) => {
                // Ensure permissions allow local microservices to write
    ...
    S: tokio::io::AsyncRead + Unpin + Send + 'static,
{
    ...
    loop {
        ...
        match reader.read_exact(&mut b1).await {
            Ok(_) => {}
            Err(_) => break, // EOF or socket closed
        }
        ...
        if b1[0] == HBP_MAGIC_0 {
            ...
            if b2[0] == HBP_MAGIC_1 {
                ...
                if header[3] == HBP_TYPE_LOG {
                    ...
                    // Fallback if payload read failed or decode failed
