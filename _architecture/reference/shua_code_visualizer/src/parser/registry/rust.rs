use std::path::Path;
use tree_sitter::{Query, QueryCursor};
use crate::graph::store::{SymbolNode, SymbolKind, Visibility, Language, AppState, SymbolTag};
use crate::parser::ast::get_parser;
use super::{DeclarationExtractor, compute_cyclomatic_complexity, infer_side_effects};

pub struct RustExtractor;

impl DeclarationExtractor for RustExtractor {
    fn extract(&self, file: &Path, source: &[u8], state: &AppState) -> Vec<SymbolNode> {
        let mut parser = match get_parser(Language::Rust) {
            Some(p) => p,
            None => return Vec::new(),
        };

        let tree = match parser.parse(source, None) {
            Some(t) => t,
            None => return Vec::new(),
        };

        let query_str = r#"
            (function_item
              (visibility_modifier)?
              name: (identifier) @name) @fn
            (struct_item
              (visibility_modifier)?
              name: (type_identifier) @name) @struct
            (enum_item
              (visibility_modifier)?
              name: (type_identifier) @name) @enum
            (trait_item
              (visibility_modifier)?
              name: (type_identifier) @name) @trait
        "#;

        let ts_lang = crate::parser::ast::get_tree_sitter_language(Language::Rust);
        let query = match Query::new(&ts_lang, query_str) {
            Ok(q) => q,
            Err(_) => return Vec::new(),
        };

        let mut cursor = QueryCursor::new();
        let matches = cursor.matches(&query, tree.root_node(), source);
        let mut symbols = Vec::new();
        let content_hash = xxhash_rust::xxh64::xxh64(source, 0);

        for mat in matches {
            let mut symbol_name = String::new();
            let mut kind = SymbolKind::Function;
            let mut line_start = 0;
            let mut line_end = 0;
            let mut complexity = 1;
            let mut signature = String::new();
            let mut visibility = Visibility::Private;
            let mut is_trait_impl = false;
            let mut fn_text = String::new();

            for cap in mat.captures {
                let cap_name = query.capture_names()[cap.index as usize];
                let node = cap.node;

                if cap_name == "name" {
                    if let Ok(text) = node.utf8_text(source) {
                        let mut qualified_name = text.to_string();
                        let mut parent = node.parent();
                        while let Some(p) = parent {
                            let kind_str = p.kind();
                            if kind_str == "impl_item" {
                                if p.child_by_field_name("trait").is_some() {
                                    is_trait_impl = true;
                                }
                                if let Some(type_node) = p.child_by_field_name("type") {
                                    if let Ok(type_name) = type_node.utf8_text(source) {
                                        let clean_type = type_name.split('<').next().unwrap_or("").trim();
                                        qualified_name = format!("{}::{}", clean_type, qualified_name);
                                    }
                                }
                                break;
                            }
                            parent = p.parent();
                        }
                        symbol_name = qualified_name;
                    }
                } else {
                    kind = match cap_name {
                        "struct" => SymbolKind::Struct,
                        "enum" => SymbolKind::Enum,
                        "trait" => SymbolKind::Trait,
                        _ => SymbolKind::Function,
                    };

                    let range = node.range();
                    line_start = range.start_point.row as u32;
                    line_end = range.end_point.row as u32;

                    if kind == SymbolKind::Function {
                        complexity = compute_cyclomatic_complexity(source, Language::Rust, node);
                    }

                    if let Ok(text) = node.utf8_text(source) {
                        let first_line = text.lines().next().unwrap_or("").trim_end();
                        signature = if first_line.ends_with('{') {
                            first_line.trim_end_matches('{').trim().to_string()
                        } else {
                            first_line.to_string()
                        };
                        if text.trim().starts_with("pub") {
                            visibility = Visibility::Public;
                        }
                        if kind == SymbolKind::Function {
                            fn_text = text.to_string();
                        }
                    }
                }
            }

            if !symbol_name.is_empty() {
                let mut intern = state.intern.write().unwrap();
                let name_key = intern.get_or_intern(&symbol_name);
                let file_key = intern.get_or_intern(file.to_string_lossy().as_ref());
                let id_key = intern.get_or_intern(format!("{}:{}", file.to_string_lossy(), symbol_name));

                let mut tags = Vec::new();
                if is_trait_impl {
                    tags.push(SymbolTag::TraitMethod);
                }
                if symbol_name == "main" && file.file_name().map(|f| f == "main.rs").unwrap_or(false) {
                    tags.push(SymbolTag::EntryPoint);
                }
                if kind == SymbolKind::Function && !fn_text.is_empty() {
                    tags.extend(infer_side_effects(&fn_text, Language::Rust));
                }

                symbols.push(SymbolNode {
                    id: id_key,
                    name: name_key,
                    file: file_key,
                    kind,
                    line_start,
                    line_end,
                    loc: line_end.saturating_sub(line_start) + 1,
                    complexity,
                    visibility,
                    signature,
                    in_degree: 0,
                    out_degree: 0,
                    is_cycle: false,
                    tags,
                    language: Language::Rust,
                    content_hash,
                });
            }
        }
        symbols
    }
}
