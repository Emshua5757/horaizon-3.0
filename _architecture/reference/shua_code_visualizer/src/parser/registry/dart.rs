...
impl DeclarationExtractor for DartExtractor {
    fn extract(&self, file: &Path, source: &[u8], state: &AppState) -> Vec<SymbolNode> {
        let mut parser = match get_parser(Language::Dart) {
            Some(p) => p,
            None => return Vec::new(),
        };
        ...
        let query_str = r#"
            ...
            (method_signature (function_signature name: (identifier) @name)) @fn
            (method_signature (getter_signature name: (identifier) @name)) @fn
            (method_signature (setter_signature name: (identifier) @name)) @fn
            (method_signature (constructor_signature name: (identifier) @name)) @fn
            (method_signature (factory_constructor_signature (identifier) @name)) @fn
        ...
        for mat in matches {
            ...
            for cap in mat.captures {
                ...
                if cap_name == "name" {
                    ...
                    if kind == SymbolKind::Function {
                        complexity = compute_cyclomatic_complexity(source, Language::Dart, node);
                    }
            ...
            if !symbol_name.is_empty() {
                ...
                if symbol_name == "main" && file.file_name().map(|f| f == "main.dart").unwrap_or(false) {
                    tags.push(SymbolTag::EntryPoint);
                }
                if kind == SymbolKind::Function && !fn_text.is_empty() {
                    tags.extend(infer_side_effects(&fn_text, Language::Dart));
                }

                let last_part = symbol_name.split('.').last().unwrap_or(&symbol_name);
                if matches!(
                    last_part,
                    "build"
                        | "initState"
                    ...
                    tags.push(SymbolTag::TraitMethod);
                }
